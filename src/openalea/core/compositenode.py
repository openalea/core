...
class CompositeNode(Node, DataFlow):
    ...
    def get_eval_algo(self, record_provenance=False, fragment_infos=None, *args, **kwargs):
        ...
        try:
            ...
            return classobj(self, record_provenance=record_provenance, fragment_infos=fragment_infos, *args, **kwargs)
    ...
    def eval_as_expression(self, vtx_id=None, step=False,
        ...
        algo = self.get_eval_algo(record_provenance, fragment_infos, *args, **kwargs)
...
class JSONCNFactoryWriter(PyCNFactoryWriter):
    def __repr__(self):
        ...
        minx = min(f.elt_ad_hoc.itervalues(), key=lambda x: x["position"][0])[
            "position"][0]
        miny = min(f.elt_ad_hoc.itervalues(), key=lambda x: x["position"][1])[
            "position"][1]
